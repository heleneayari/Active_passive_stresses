function [hh,autoscale] = quiverh(varargin)
%QUIVER Quiver plot.
...
% le autoscale renvoyé est bien l'autoscale final multiplié par le facteur
% correctif!!
...
h1 = plot(uu(:),vv(:),[col ls],'LineWidth',1.2);
...
if plotarrows,
  ...
  hold on
  h2 = plot(hu(:),hv(:),[col ls],'LineWidth',1.2);
else
  h2 = [];
end
